...
import { RefreshCw, Play, Hash, Terminal as TerminalIcon, Trash2, Info } from 'lucide-react';
...
const PodsPage = ({ namespace, onOpenTerminal, onViewDetail }: Props) => {
    ...
    <div className="space-y-8 relative">
      <div className="flex items-end justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-black tracking-tighter uppercase italic">Pods</h1>
        </div>
        <button 
          onClick={fetchPods}
          disabled={loading}
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="font-mono text-sm font-bold text-gray-200 group-hover:text-white transition-colors">{pod.name}</span>
                        <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">
                          UUID: {pod.name.split('-').pop()}
                        </span>
                      </div>
                    </div>
                  </td>
