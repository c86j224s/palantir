...
import ResourceDetail from './components/ResourceDetail'; // 새로 추가할 상세 패널
    ...
    <div className="flex h-screen bg-background text-foreground overflow-hidden font-sans">
      ...
      <aside className="w-[72px] hover:w-64 border-r border-border flex flex-col p-4 bg-card/80 backdrop-blur-3xl transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] group z-50">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto overflow-x-hidden custom-scrollbar pr-2">
          ...
          <div className="py-2 px-3 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] opacity-0 group-hover:opacity-100 transition-opacity">Workloads</div>
          ...
          <div className="flex items-center gap-8">
            <div className="relative group">
              <button 
                ...
                className="flex items-center gap-3 bg-secondary/50 hover:bg-secondary px-5 py-2.5 rounded-2xl border border-border transition-all active:scale-95 shadow-sm"
              >
                ...
                {isNsOpen && (
                  <>
                    ...
                    <motion.div 
                      ...
                      className="absolute top-full left-0 mt-3 w-64 bg-popover border border-border rounded-3xl p-3 z-20 shadow-2xl ring-1 ring-white/10"
                    >
                        ...
                        {namespaces.map(ns => (
                          <button
                            ...
                            className={`w-full text-left px-4 py-2.5 rounded-xl text-sm transition-all ${
                              selectedNamespace === ns ? 'bg-primary/10 text-primary font-bold' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'
                            }`}
                          >
                            {ns}
                          </button>
...
const SidebarItem = ({ icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) => (
  <button 
    ...
    className={`w-full flex items-center gap-4 px-3 py-3 rounded-2xl transition-all duration-300 group relative ${
      active ? 'bg-primary/10 text-primary shadow-[inset_0_0_20px_hsl(var(--primary)/0.1)]' : 'text-muted-foreground hover:text-foreground hover:bg-secondary/50'
    }`}
  >
